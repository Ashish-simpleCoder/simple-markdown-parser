...
export function parseList(inputs: string[], parseListItemContent: (content: string) => string) {
    ...
    listMap.set(ROOT_ID, { parent: null, children: [] })

    let lastInsertedItem: ListItem | null = null

    inputs.forEach((line, id) => {
        id = id + 1
        let listTypeToGenerate: ListTypeToGenerate | undefined
        ...
        let newItemToInsert: ListItem = null
    ...
    if (listMap.size > 0) {
        const fragment = generateFullHtmlTree(listMap, listMap.get(ROOT_ID) as ListItem)
        if (fragment) {
...
function generateFullHtmlTree(map: HtmlMap, rootElementObj: ListItem) {
    ...
    while (childIds.length > 0) {
        ...
        let currentChild = map.get(childIds.shift() as any)
        ...
        if (!lastParentTagGenerated) {
            ...
            // if parent or list-element type changes
            // prettier-ignore
...
function findParent_BasedOn_WhitespaceLength_Of_CurrentItem_ToBeInserted({ htmlMap, lastInsertedItem, currentListItemToBeInserted_Whitespace_Length: currentListItem_Whitespace_Length }: { htmlMap: HtmlMap, lastInsertedItem: ListItem, currentListItemToBeInserted_Whitespace_Length: number }) {
    let parent: ListItem | undefined = lastInsertedItem
